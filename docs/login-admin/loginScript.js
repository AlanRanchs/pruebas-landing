class AdminLogin {
  ...
  setupPasswordToggle() {
    ...
    this.passwordToggle.addEventListener("click", () => {
      ...
      this.passwordToggle.setAttribute(
        "aria-label",
        isPasswordHidden ? "Ocultar contraseña" : "Mostrar contraseña"
      );
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch(
        "https://pruebas-landing.onrender.com/api/auth/login",
        {
          method: "POST",
  ...
  clearAllErrors() {
    ["emailGroup", "passwordGroup"].forEach((id) =>
  ...
  resetButton() {
    setTimeout(() => {
      ...
      this.submitButton.style.borderColor = "";
      document.getElementById("buttonText").textContent = "Sign In";
    }, 2000);
  }
}
