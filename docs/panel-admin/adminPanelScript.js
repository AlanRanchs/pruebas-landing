...
function updateSummaryAndTotal() {
  ...
  servicesSummary.innerHTML = paquete.servicios
    .map((s) => `<li>${s}</li>`)
    .join("");
  ...
  salesTable.innerHTML =
    filtered.length > 0
      ...
      <tr class="border-top text-sm">
        ...
        <td class="p-2 text-green-600 font-semibold">$${parseFloat(
          sale.monto
        ).toLocaleString()}</td>
      </tr>
    `
...
async function fetchSales() {
  try {
    ...
    const res = await fetch(
      "https://pruebas-landing.onrender.com/api/adminPanel/ventas",
      {
        headers: {
...
saleForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const token = localStorage.getItem("adminToken");
    const res = await fetch(
      "https://pruebas-landing.onrender.com/api/adminPanel/venta-manual",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
document
  .getElementById("exportToSheetsButton")
  .addEventListener("click", async () => {
    try {
      ...
      const res = await fetch(
        "https://pruebas-landing.onrender.com/api/exportar-ventas",
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
